...
Так что же такое ViewModel - это `UIKit` независимая прослойка между моделью и представлением. Она получает обновления от модели и обновляет свое состояние и модель, а так как ViewModel имеет связь еще и с представлением, то обновляет и его. 

MVVM, в отличие от MVP, предполагает использование так называемого связывания данных (биндингов). При этом связывается как `ViewModel` и `View`, так и `ViewModel` и `Model`.    

MVVM предполагает использование такого подхода как `реактивное программирование`. Самый известный фрейморк, реализующий эту парадигму - `RxSwift`, имеющий расширение `RxCocoa` - для работы с компонентами `Cocoa` и `CocoaTouch` как с реактивными. При использовании Rx-фреймворков существенно усложняется как код проекта так и его отладка (стек вызовов превращается в кашу). Так что лучше не использовать эти фреймворки в работе или же ограничить их использование.
